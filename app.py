...
def mocks():
    ...
    return render_template(
        ...
        trend_chart_url=chart_url(trend_chart),
        pie_chart_url=chart_url(pie_chart),
    )


def generate_charts(profile_id):
    """Redraw the trend and pie chart PNGs for a profile.

    Each chart is drawn inside its own try/except so one chart failing
    doesn't stop the mock test save or the other chart from working.
    """
    db = get_db()
    # No LIMIT here - every mock test this profile has ever logged is
    # pulled in, oldest to newest, so the trend chart below always
    # reflects the FULL history, not just the most recent few.
    rows = db.execute(
    ...
    # --- Score trend line chart: every logged mock, oldest to newest ---
    try:
        dates = [r["date"] for r in rows]
        scores = [mock_total_score(r) for r in rows]
        labels = [d[5:] for d in dates]  # MM-DD for the x-axis

        fig, ax = plt.subplots(figsize=(6, 2.8), dpi=150)
        fig.patch.set_facecolor(PAPER)
        ax.set_facecolor(PAPER)
        ax.plot(labels, scores, color=INK, linewidth=2, marker="o", markersize=5,
        ...
        ax.grid(color=LIGHT_GREY, linewidth=0.7, axis="y")
        ...
        if len(labels) > 8:
            # Too many mocks to fit every date label legibly - thin them
            # out on the axis, but the plotted line still includes every
            # single mock point, none are dropped from the data itself.
